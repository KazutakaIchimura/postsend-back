...
class DashboardControllerTest {
    ...
    @BeforeEach
    void setUp() {
        // 各テストで個別に DashboardResponse をスタブする
    }
    ...
    class GetDashboard {
        ...
        void no87_pendingCount_aggregatedCorrectly() throws Exception {
            DashboardResponse response = DashboardResponse.builder()
                    .pendingCount(5L).overdueCount(0L).sentThisMonthCount(0L)
        ...
        void no88_overdueCount_aggregatedCorrectly() throws Exception {
            DashboardResponse response = DashboardResponse.builder()
                    .pendingCount(5L).overdueCount(3L).sentThisMonthCount(0L)
                    .currentMonth("2026-06")
                    .overdueMonths(List.of())
                    .recentHistory(List.of())
                    .build();
        ...
        void no89_sentThisMonthCount_aggregatedCorrectly() throws Exception {
            DashboardResponse response = DashboardResponse.builder()
                    .pendingCount(0L).overdueCount(0L).sentThisMonthCount(7L)
                    .currentMonth("2026-06")
                    .overdueMonths(List.of())
                    .recentHistory(List.of())
                    .build();
        ...
        void no90_overdueMonths_returnedInAscendingOrder() throws Exception {
            DashboardResponse response = DashboardResponse.builder()
                    .pendingCount(2L).overdueCount(2L).sentThisMonthCount(0L)
                    .currentMonth("2026-06")
                    .overdueMonths(List.of(
                            DashboardResponse.OverdueMonthCount.builder().month("2026-03").count(1L).build(),
                            DashboardResponse.OverdueMonthCount.builder().month("2026-04").count(1L).build()
                    ))
                    .recentHistory(List.of())
                    .build();
            when(dashboardService.getDashboard()).thenReturn(response);
        ...
        void no91_recentHistory_returnsUpTo5ItemsInDescendingSentAtOrder() throws Exception {
            ...
            DashboardResponse response = DashboardResponse.builder()
                    .pendingCount(0L).overdueCount(0L).sentThisMonthCount(2L)
                    .currentMonth("2026-06")
                    .overdueMonths(List.of())
                    .recentHistory(List.of(
                            DashboardResponse.RecentHistoryItem.builder()
                                    .id(2L).officeName("中央事業所").userName("山田太郎")
                                    .sendType(SendType.PLAN).sentAt(latest).build(),
                            DashboardResponse.RecentHistoryItem.builder()
                                    .id(1L).officeName("南事業所").userName("鈴木花子")
                                    .sendType(SendType.MONITORING).sentAt(older).build()
                    ))
                    .build();
            when(dashboardService.getDashboard()).thenReturn(response);
        ...
        void no92_noData_returnsEmptyArraysAndZeroCounts() throws Exception {
            DashboardResponse response = DashboardResponse.builder()
                    .pendingCount(0L).overdueCount(0L).sentThisMonthCount(0L)
                    .currentMonth("2026-06")
                    .overdueMonths(List.of())
                    .recentHistory(List.of())
                    .build();
